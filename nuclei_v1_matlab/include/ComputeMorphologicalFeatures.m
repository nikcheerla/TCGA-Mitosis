function [MorphFeatures, NucleiCentroids] = ComputeMorphologicalFeatures( BW, FeaturesPath, ImageName )
%ComputeMorphologicalFeatures compute morphological features
    ...
    for i = 1:numel(morphFeatures)
        ...
        MorphFeatures(i).Ellipse_X(i,1) = 2 * sqrt(S(1,1)); % eigen-values are a measure of variance
        MorphFeatures(i).Ellipse_Y(i,1) = 2 * sqrt(S(2,2)); % eigen-values are a measure of variance
    end
    
    if(nargin == 3)
        struct2csv(NucleiCentroids,strcat(FeaturesPath,ImageName,'_Centroid.csv'));
        struct2csv(MorphFeatures,strcat(FeaturesPath,ImageName,'_MorphologicalFeatures.csv'));
    end
