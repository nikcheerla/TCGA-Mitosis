...
def train_icpr(params_dict):
    ...
    for imgfile in glob.iglob("data/train_icpr/*.bmp"):
        print("\n" + imgfile, end="")
    ...
    img2 = np.append(img, np.append(img, img, axis = 1), axis = 1)
    img3 = np.append(img2, np.append(img2, img2, axis = 2), axis = 2)
    ...
    def get_patches(coords, patchsize=PATCH_SIZE):
        ...
        for (x, y, img_num) in coords:
            ...
            patches[i] = img3[img_num, (x - patchsize / 2):(x + patchsize / 2 + 1),
